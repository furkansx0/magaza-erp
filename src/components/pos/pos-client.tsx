...
import { Trash2, ShoppingCart, Banknote, CheckCircle2, Loader2, Receipt, RefreshCw, Truck, Search as SearchIcon, Coins, Tag } from "lucide-react"
...
export function PosClient({ staffList, storeName, stores, currentUserRole, currentUserStoreId, campaigns }: {
    ...
    const addToCart = (product: PosProduct) => {
        ...
        if (!isReturn && product.stock <= 0) { toast.error("Stokta ürün kalmadı!"); return; }

        let qtyToAdd = 1;
        if (isExchangeMode) qtyToAdd = exchangeTarget === 'INCOMING' ? -1 : 1;

        setCart(prev => {
            const existing = prev.find(item => item.variantId === product.variantId)
            if (existing) {
                const newQty = existing.quantity + qtyToAdd;
                if (qtyToAdd > 0 && existing.quantity >= product.stock) { toast.error(`Stok yetersiz! (Mevcut: ${product.stock})`); return prev; }
                if (newQty === 0) return prev.filter(i => i.variantId !== product.variantId);
                return prev.map(item => item.variantId === product.variantId ? { ...item, quantity: newQty } : item)
            }
            return [{
                ...product,
                quantity: qtyToAdd,
        ...
        if (isExchangeMode) msg = qtyToAdd < 0 ? "İade ürünü eklendi (Gelen)" : "Satış ürünü eklendi (Giden)";
    ...
    const updateQuantity = (variantId: string, qty: number) => {
        setCart(prev => prev.map(item => {
            ...
            if (qty > 0 && qty > item.stock) { toast.error(`Stok yetersiz! (Maks: ${item.stock})`); return item; }
            return { ...item, quantity: qty }
        }))
    }

    const updateSalesRep = (variantId: string, staffId: string) => {
        setCart(prev => prev.map(item => item.variantId === variantId ? { ...item, salesRepId: staffId } : item));
    }

    const updateFinalPrice = (variantId: string, newPrice: number) => {
        setCart(prev => prev.map(item => item.variantId === variantId ? { ...item, finalPrice: newPrice >= 0 ? newPrice : 0 } : item))
    }

    const restoreOriginalPrice = (variantId: string) => {
        setCart(prev => prev.map(item => item.variantId === variantId ? { ...item, finalPrice: item.originalPrice } : item))
    }

    const removeFromCart = (variantId: string) => setCart(prev => prev.filter(item => item.variantId !== variantId))

    const clearCart = () => {
        if (confirm("Sepeti temizlemek istediğinize emin misiniz?")) { setCart([]); setSelectedCustomer(null); }
    }

    React.useEffect(() => { setCart([]) }, [selectedStoreId])
    ...
    const handleOpenPayment = () => {
        ...
        if (!isExchangeMode) {
            ...
            if (missingRep) { toast.error(`"${missingRep.modelName}" için satış temsilcisi seçilmedi!`); return; }
        }
        if (isExchangeMode && exchangeBalance <= 0) { handleCompletePayment([], true); return; }
    ...
    return (
        <div className="flex flex-col h-[calc(100vh-4rem)] bg-gray-50 dark:bg-gray-950">
            ...
            <div className="flex items-center gap-4 bg-white dark:bg-gray-900 border-b p-4 shadow-sm shrink-0 z-30">
                ...
                <div className="flex items-center gap-2">
                    <Button variant="outline" onClick={() => setExpenseOpen(true)}
                        ...
                        <Coins className="h-4 w-4" /> Gider Ekle
                    </Button>
                    <Button variant="outline" onClick={() => setCashClosingOpen(true)}
                        ...
                        <Banknote className="h-4 w-4" /> Nakit Sayımı
                    </Button>
                    <Button onClick={() => setTransferOpen(true)} className="bg-blue-600 hover:bg-blue-700 text-white font-semibold h-10 px-4 shadow-sm">
                        <Truck className="mr-2 h-4 w-4" /> Hızlı Transfer
                    </Button>
                    <Button onClick={() => setStockQueryOpen(true)} className="bg-sky-500 hover:bg-sky-600 text-white font-semibold h-10 px-4 shadow-sm">
                        <SearchIcon className="mr-2 h-4 w-4" /> Stok Sor
                    </Button>
                </div>
                ...
                <div className="shrink-0 z-20 flex flex-col gap-2">
                    ...
                    {isExchangeMode && (
                        <div className="flex gap-4 animate-in slide-in-from-top-2">
                            <div onClick={() => setExchangeTarget('INCOMING')}
                                className={cn("flex-1 p-3 rounded-lg border-2 cursor-pointer transition-all flex items-center justify-between",
                                    exchangeTarget === 'INCOMING' ? "bg-red-50 border-red-500 shadow-sm" : "bg-white border-transparent hover:bg-gray-50 opacity-60")}>
                                <div className="flex items-center gap-2 font-bold text-red-700">
                                    <RefreshCw className="h-5 w-5" /> GELEN ÜRÜN (İADE)
                                </div>
                                <span className="text-xs bg-red-100 text-red-800 px-2 py-1 rounded-full">Buraya Ekleniyor</span>
                            </div>
                            <div onClick={() => setExchangeTarget('OUTGOING')}
                                className={cn("flex-1 p-3 rounded-lg border-2 cursor-pointer transition-all flex items-center justify-between",
                                    exchangeTarget === 'OUTGOING' ? "bg-green-50 border-green-500 shadow-sm" : "bg-white border-transparent hover:bg-gray-50 opacity-60")}>
                                <div className="flex items-center gap-2 font-bold text-green-700">
                                    <ShoppingCart className="h-5 w-5" /> GİDEN ÜRÜN (SATIŞ)
                                </div>
                            ...
                            <div className="flex gap-4 h-full p-2">
                                ...
                                <div className="flex-1 border rounded-lg overflow-hidden flex flex-col bg-red-50/30 border-red-100">
                                    <div className="p-2 bg-red-100 text-red-800 font-bold text-center border-b border-red-200 flex items-center justify-center gap-2">
                                        <RefreshCw className="h-4 w-4" /> İADE EDİLENLER (GELEN)
                                        ...
                                        <Table>
                                            ...
                                            <TableBody>
                                                ...
                                                {returnsList.length === 0 && (
                                                    <TableRow><TableCell colSpan={5} className="text-center text-muted-foreground h-24">Henüz iade ürün eklenmedi</TableCell></TableRow>
                                                )}
                                            </TableBody>
                                        </Table>
                                    </div>
                                ...
                                <div className="flex-1 border rounded-lg overflow-hidden flex flex-col bg-green-50/30 border-green-100">
                                    <div className="p-2 bg-green-100 text-green-800 font-bold text-center border-b border-green-200 flex items-center justify-center gap-2">
                                        <ShoppingCart className="h-4 w-4" /> YENİ ÜRÜNLER (GİDEN)
                                    ...
                                    <div className="flex-1 overflow-auto">
                                        <Table>
                                            ...
                                            <TableBody>
                                                ...
                                                {salesList.length === 0 && (
                                                    <TableRow><TableCell colSpan={5} className="text-center text-muted-foreground h-24">Henüz yeni ürün eklenmedi</TableCell></TableRow>
                                                )}
                                            </TableBody>
                                        </Table>
                                    </div>
                                    ...
                                    {cart.map(item => (
                                        <TableRow key={item.variantId}>
                                                ...
                                                <div className="flex flex-col">
                                                    ...
                                                    {campaignDiscounts.some(d => d.matchedItemIds.includes(item.variantId)) && (
                                                        <span className="text-[10px] text-green-600 flex items-center gap-1 mt-0.5 font-bold">
                                                            <Tag className="w-3 h-3" /> Kampanyalı Ürün
                                                        </span>
                                                    )}
                                                </div>
                                            </TableCell>
                                            <TableCell>
                                                <Select value={item.salesRepId || ""} onValueChange={(val) => updateSalesRep(item.variantId, val)}>
                                                    <SelectTrigger className="h-8 border-gray-200 bg-gray-50/50">
                                                        <SelectValue placeholder="Personel Seç" />
                                                    </SelectTrigger>
                                                    <SelectContent>
                                                        {filteredStaffList.map(staff => (
                                                            <SelectItem key={staff.id} value={staff.id}>{staff.name || staff.username}</SelectItem>
                                                        ))}
                                                    </SelectContent>
                                                </Select>
                                            </TableCell>
                                            ...
                                            <TableCell className="text-right font-bold">
                                                {new Intl.NumberFormat('tr-TR', { style: 'currency', currency: 'TRY' }).format(item.finalPrice * item.quantity)}
                                            </TableCell>
                                            <TableCell>
                    ...
                    <div className="p-4 bg-gray-50 dark:bg-gray-900 border-t space-y-4">
                        <div className="flex justify-between items-end">
                            ...
                            <div className="text-right">
                                <div className="text-sm text-muted-foreground mb-1">{!isExchangeMode ? "GENEL TOPLAM" : "ÖDENECEK FARK"}</div>
                                {isExchangeMode && exchangeBalance <= 0 ? (
                                    <div className="text-3xl font-black tracking-tight text-gray-400">0,00 ₺</div>
                                ) : (
                        ...
                        {totalCampaignDiscount > 0 && !isExchangeMode && (
                            <div className="flex justify-between items-center text-green-600 bg-green-50 p-2 rounded-md animate-pulse">
                                <span className="flex items-center gap-1 font-semibold"><Tag className="w-4 h-4" /> Kampanya İndirimi Uygulandı</span>
                        ...
                        <div className="h-14">
                            <Button
                                className={cn("w-full h-full text-xl font-bold shadow-lg dark:shadow-none transition-colors",
                                    (isExchangeMode && exchangeBalance <= 0) ? "bg-green-600 hover:bg-green-700 shadow-green-200"
                                        : totalAmount < 0 ? "bg-orange-600 hover:bg-orange-700 shadow-orange-200"
                            ...
                            >
                                ...
                                {!isExchangeMode ? (totalAmount < 0 ? "DEĞİŞİMİ TAMAMLA (İADESİZ)" : "ÖDEME AL")
                                    : (exchangeBalance <= 0 ? "İŞLEMİ TAMAMLA" : "ÖDEME AL (FARK TAHSİLATI)")}
                            </Button>
                        </div>
                    </div>
                </div>
            ...
            {/* Dialogs */}
            <StoreExpenseDialog open={expenseOpen} onOpenChange={setExpenseOpen} storeId={selectedStoreId} />
            <CashClosingDialog open={cashClosingOpen} onOpenChange={setCashClosingOpen} storeId={selectedStoreId} staffId={filteredStaffList[0]?.id || ""} />
            <PaymentDialog open={isPaymentDialogOpen} onOpenChange={setIsPaymentDialogOpen} totalAmount={totalAmount - totalCampaignDiscount} onComplete={handleCompletePayment} isLoading={checkoutLoading} customer={selectedCustomer} />
            <StockQueryDialog open={stockQueryOpen} onOpenChange={setStockQueryOpen} />
            <QuickTransferDialog open={transferOpen} onOpenChange={setTransferOpen} currentStoreId={selectedStoreId} allStores={stores || []} currentStaffId={filteredStaffList[0]?.id || ""} />
                ...
                <DialogContent className="sm:max-w-md bg-green-50 dark:bg-green-900/20 border-green-200 dark:border-green-800">
                    ...
                    <DialogFooter className="sm:justify-center gap-2 pb-4">
                        <Button variant="outline" className="border-green-200 text-green-700 hover:bg-green-100"
                            onClick={() => { if (lastSaleId) window.open(`/pos/receipt/${lastSaleId}`, 'Receipt', 'width=400,height=600') }}>
                            <Receipt className="mr-2 h-4 w-4" /> Bilgi Fişi Yazdır
                        </Button>
            ...
            <Dialog open={!!receiptUrl} onOpenChange={(open) => !open && setReceiptUrl(null)}>
                <DialogContent className="max-w-[400px] h-[80vh] p-0 overflow-hidden bg-white">
                    {receiptUrl && <iframe src={receiptUrl} className="w-full h-full border-0" title="Receipt Preview" />}
                </DialogContent>
            </Dialog>
        </div>
    )
